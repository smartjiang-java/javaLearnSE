...
3：领域建模
 ...
 2：根据聚合根的管理性质从实体中找出集合根。
 3：划定限界上下文，根据上下文语义将聚合归类。

4：服务拆分
 用户接口层：用户界面，web服务，其他--->接口适配
 应用层：应用服务                ---->
 领域层：聚合（实体，值对象），领域服务
 基础层：数据库，事件总线，API网关，缓存，第三方工具和插件，基础服务等

DDD 强调先构建领域模型然后设计微服务，以保证领域模型和微服务的一致性。

设计领域层：
1：设计实体：大多数情况下，领域模型的业务实体与微服务的数据库实体是一一对应的。
2：找出聚合根：聚合根可以实现聚合之间的对象引用，还可以引用聚合内的所有实体，有自己的实现方法
3：设计值对象：根据需要将某些实体的某些属性或属性集设计为值对象
4：设计领域事件
5：设计领域服务：一个业务动作或行为跨多个实体，就需要设计领域服务
6：设计仓储：一个聚合一个仓储，完成数据查询和持久化操作
